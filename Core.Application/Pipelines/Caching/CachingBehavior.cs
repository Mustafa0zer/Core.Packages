    ...
    where TRequest : IRequest<TResponse>, ICachableRequest
{
    ...
    private async Task<TResponse?> getResponseAndAddToCache(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        TimeSpan? slidingExpiration = request.SlidingExpiration?? TimeSpan.FromDays(_cacheSettings.SlidingExperation);
        ...
        if(request.CacheGroupKey != null)
            await addCacheKeyToGroup(request,slidingExpiration.Value,cancellationToken);
...
//Cache Data
//CacheGroupKey - cacheKeys[]-->string key listesi.